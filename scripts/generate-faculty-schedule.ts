...
import {CS_GROUP_IDS,teacherKey} from "../src/lib/cs-beta-data";
...
const canonicalTeacherIds=new Map<string,string>();
function teacherId(teacher: string) {
  const existing=canonicalTeacherIds.get(teacherKey(teacher));
...
export async function generateSemesterSchedule() {
  ...
  if(hasFlag("replace"))throw new Error("--replace is disabled: attendance history must be preserved.");
  ...
  const knownGroupIds =
    new Set(
      groups.filter(group=>!CS_GROUP_IDS.includes(group.id as typeof CS_GROUP_IDS[number])).map(
        (group) => group.id,
      ),
    );
   ...
   */
  for(const teacher of await db.teacher.findMany())canonicalTeacherIds.set(teacherKey(teacher.displayName),teacher.id);
  for (
    ...
    if (
      ...
      for (
        ...
        for (
          ...
          if(!student.isSynthetic&&atKyiv(isoDate,endTime)<=effectiveNow().toJSDate()&&student.joinedAt>atKyiv(isoDate,startTime))continue;
          roster.push({
  ...
  console.log("");
  console.log(
    "Existing lessons and attendance were preserved; CS groups are excluded.",
  );
