...
async function main(){
  ...
  const roleLabels:Record<RoleCode,string>={ADMIN:'Адміністратор',DEAN_OFFICE:'Деканат',CURATOR:'Куратор',TEACHER:'Викладач',STAROSTA:'Староста'};
  ...
  for(const cell of cells){
    ...
    if(cell.teacher)await db.teacher.upsert({where:{id:id('teacher',cell.teacher)},create:{id:id('teacher',cell.teacher),displayName:cell.teacher,source:json({sourceId:cell.id,file:cell.file,page:cell.page,raw:cell.raw})},update:{}});
  ...
  for(let gi=0;gi<groups.length;gi++)for(let si=0;si<15;si++){
    const g=groups[gi];const nameIndex=(si+gi)%15;
  ...
  const starosta=await createUser('starosta@test.com','Староста · демо',['STAROSTA']);
  if(starosta.created)await db.student.update({where:{id:`${kn.id}-student-1`},data:{userId:starosta.user.id}});
  ...
  async function addLesson(lessonId:string,gids:string[],date:string,template:Cell,history:number|null,cancelled=false){
    ...
    const teacherId=template.teacher?id('teacher',template.teacher):null;
    let pair=1;
    ...
    const students=await db.student.findMany({where:{groupId:{in:gids}},orderBy:{id:'asc'}});
    ...
    await db.$transaction(async tx=>{
      ...
      if(past && history!==null && !empty && !cancelled){
        const marks=students.flatMap((s,i)=>{
          ...
          return [{studentId:s.id,lessonId,statusCode,confirmed:true}];
        });
  ...
  for(let gi=0;gi<groups.length;gi++){
    const g=groups[gi];const options=usable.filter(c=>c.groups.includes(g.id));
  ...
  if(joint)await addLesson('demo-joint-kn3',joint.groups,today(),joint,null);
  for(let i=0;i<teachers.length;i++){
    const t=teachers[i];
    if(!await db.lesson.count({where:{teacherId:t.id}}))continue;
    const email=t.displayName==='Пархоменко О.Ю.'?'teacher@test.com':`teacher-${String(i+1).padStart(2,'0')}@test.com`;
