...
const input=z.discriminatedUnion('action',[
 z.object({action:z.literal('CREATE'),name:z.string().trim().min(2).max(100),facultyId:z.string().max(100),fromDate:date,toDate:date,confirm:z.literal(true),reason:z.string().trim().min(5).max(500)}).strict(),
...
export async function changeTerm(user:Principal,raw:unknown){const parsed=input.safeParse(raw);if(!parsed.success)throw new HttpError(400,'Перевірте дати, причину та підтвердження.');const p=parsed.data;return db.$transaction(async tx=>{
 ...
 await tx.auditLog.create({data:{actorId:actor.id,objectType:'AcademicTerm',objectId:id,source:'TERM_'+p.action,reason:p.reason,details:JSON.parse(JSON.stringify(p))}});return {ok:true,id};
},{timeout:60000});}
