...
export function reportXlsx(summary:ReportSummary,f:ReportFilters){return xlsx([{name:'Заняття',rows:reportTable(summary,f)},{name:'Студенти',rows:[['TEST','ПІБ','Група','PRESENT','N','HV','Не відмічено','Показник %'],...summary.rows.map(s=>['TEST',s.fullName,s.groupName,s.stats.PRESENT,s.stats.N,s.stats.HV,s.stats.unmarked,s.stats.percentage??'Немає даних'])]}]);}
export async function reportFiles(summary:ReportSummary,f:ReportFilters){
  ...
  text('TEST · MNAU Attendance',18);text(`${f.from} — ${f.to} · ${summary.scope}`,11);
