...
import {z} from 'zod';
import {db} from './db';
import {hasRole,starostaGroups,principalSelect,type Principal} from './auth';
...
import {journalRowSchema,resolveAttendanceMode} from './attendance-mode';

const schema=z.object({version:z.number().int().min(0),requestId:z.uuid(),mode:z.enum(['DRAFT','CONFIRM','AUTO']),reason:z.string().trim().max(500).optional(),rows:z.array(journalRowSchema).min(1).max(500)}).strict();
...
export async function journal(user:Principal,id:string,client:Prisma.TransactionClient=db){
  ...
  const superseded=lesson.synthetic&&!!await client.systemState.findUnique({where:{id:'cs-beta'}})&&!!await client.lesson.count({where:{AND:[{id},obsoleteCSCalendar]}});
...
export async function saveJournal(user:Principal,id:string,raw:unknown){
  ...
  return db.$transaction(async tx=>{
    ...
    for(const row of input.rows){
      ...
      const attendanceMode=resolveAttendanceMode(row,old?.attendanceMode??null);
      if(row.status===null){await tx.attendance.deleteMany({where:{lessonId:id,studentId:row.studentId}});}else{await tx.attendance.upsert({where:{studentId_lessonId:{studentId:row.studentId,lessonId:id}},create:{studentId:row.studentId,lessonId:id,statusCode:row.status,attendanceMode,confirmed},update:{statusCode:row.status,attendanceMode,confirmed,isDemo:false,source:'MANUAL',batchId:null}});}
      if((old?.statusCode??null)!==row.status || (old?.attendanceMode??null)!==attendanceMode || (!!old?.confirmed)!==(row.status!==null&&confirmed))await tx.auditLog.create({data:{actorId:actor.id,lessonId:id,studentId:row.studentId,objectType:'Attendance',objectId:`${id}:${row.studentId}`,oldStatus:old?.statusCode??null,newStatus:row.status,reason:input.reason??null,source:state.needsReason?'AUTHORIZED_CORRECTION':hasRole(actor,'STAROSTA')?'STAROSTA_SAVE':'JOURNAL_SAVE',details:{oldConfirmed:old?.confirmed??false,newConfirmed:row.status!==null&&confirmed,oldAttendanceMode:old?.attendanceMode??null,newAttendanceMode:attendanceMode,requestId:input.requestId}}});
    }
