...
export async function analytics(user:Principal,f:Filters,client:Prisma.TransactionClient=db){
  ...
  return {groups:enrichedGroups,specialties,students,lessonCount:lessons.length,stats:metrics(sumCounts(students.map(s=>s.stats))),below70:studentTotals(students).filter(s=>s.stats.below70).length,below50:studentTotals(students).filter(s=>s.stats.below50).length};
}
