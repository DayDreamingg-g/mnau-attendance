...
import {isManager,principalSelect,type Principal} from './auth';
...
export async function adminChange(actor:Principal,body:unknown){
  ...
  if(p.action==='RESET_BETA_PASSWORD'&&!demoEnabled())throw new HttpError(403,'Beta-пароль доступний лише у demo.');
  const password=reset?(p.action==='RESET_BETA_PASSWORD'?'Test1234!':randomBytes(18).toString('base64url')):undefined;
  ...
  return db.$transaction(async tx=>{
    ...
    if(!currentActor?.active||!isManager(currentActor))throw new HttpError(403,'Потрібні права адміністратора або розробника.');
    ...
    if(p.action==='ADD_ROLE'||p.action==='REMOVE_ROLE'){
      ...
      if(p.action==='RESET_BETA_PASSWORD'&&!demoEnabled())throw new HttpError(403,'Beta-пароль вимкнено.');
      await tx.user.update({where:{id:p.userId},data:{passwordHash}});
