export type Counts = { PRESENT:number; N:number; HV:number; unmarked:number; pending:number; ONLINE?:number; OFFLINE?:number };
export const emptyCounts = ():Counts => ({PRESENT:0,N:0,HV:0,unmarked:0,pending:0,ONLINE:0,OFFLINE:0});
export function countAttendance(c:Counts,a:{statusCode:'PRESENT'|'N'|'HV';attendanceMode?:'ONLINE'|'OFFLINE'|null}|null){
  if(!a){c.unmarked++;return;}
  c[a.statusCode]++;
  if(a.statusCode==='PRESENT'&&a.attendanceMode)c[a.attendanceMode]=(c[a.attendanceMode]??0)+1;
}
...
export function sumCounts(values:Counts[]):Counts {return values.reduce((a,b)=>({PRESENT:a.PRESENT+b.PRESENT,N:a.N+b.N,HV:a.HV+b.HV,unmarked:a.unmarked+b.unmarked,pending:a.pending+b.pending,ONLINE:(a.ONLINE??0)+(b.ONLINE??0),OFFLINE:(a.OFFLINE??0)+(b.OFFLINE??0)}),emptyCounts());}
