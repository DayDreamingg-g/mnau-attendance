...
export function ReportForm({filters,options,periods}:{filters:ReportFilters;options:ReportOptions;periods:Record<ReportKind,{from:string;to:string}>}){
  ...
  const [kind,setKind]=useState<ReportKind>('WEEKLY'),[from,setFrom]=useState(filters.from),[to,setTo]=useState(filters.to);
  ...
  const values={faculty,kind,from,to,course,specialty:specialty||(options.specialties.length===1&&options.specialties[0].facultyId===faculty?options.specialties[0].id:''),group,student,threshold,subject,term:filters.term,scope:filters.scope};
  ...
  async function submit(){
    ...
    try{
      ...
      setMessage(result.state==='PENDING'?'Звіт формується. Оновіть список за мить.':'Звіт створено');if(result.state==='READY')router.push('/reports/'+result.id);router.refresh();
  ...
  return <><form className="filters report-filters" onSubmit={e=>{e.preventDefault();void submit();}}>
    ...
    <label>Дисципліна<select value={subject} onChange={e=>setSubject(e.target.value)} disabled={busy}><option value="">Усі дисципліни</option>{options.subjects?.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label><label>Поріг<CustomSelect name="threshold" label="Поріг" value={threshold} disabled={busy} options={[{value:'',label:'Усі показники'},{value:'70',label:'Нижче 70%'},{value:'50',label:'Нижче 50%'}]} onChange={v=>{setThreshold(v);changed();}}/></label>
    <button type="button" className="button" disabled={busy} onClick={()=>{setKind('WEEKLY');setFrom(periods.WEEKLY.from);setTo(periods.WEEKLY.to);}}>Поточний тиждень</button><button className="button" disabled={busy||!faculty}>{busy?'Створення...':'Створити звіт'}</button>
