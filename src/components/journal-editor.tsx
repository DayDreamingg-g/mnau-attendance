...
type Row={isDemo?:boolean;id:string;name:string;group:string;status:JournalStatus;confirmed:boolean;editable:boolean;canConfirm:boolean};
...
function JournalForm({lessonId,initialVersion,rows,needsReason:initialNeedsReason,filters}:Props){
  ...
  return <section className="panel" aria-busy={busy}>
    <div className="journal-toolbar"><div><h2>Відмітки студентів</h2><p className="muted small">• Присутність · N Відсутній · HV Поважна причина</p><p className="muted small">У рядку: клавіші 1 / 2 / 3 / 0. Заповнено: {draft.filter(row=>row.status!==null).length} із {draft.length}.</p></div><button type="button" className="button" disabled={!editable||busy} onClick={()=>{setDraft(list=>list.map(row=>row.editable?{...row,status:'PRESENT'}:row));setSuccess('');setConfirming(false);}}>Позначити всіх присутніми</button></div>
