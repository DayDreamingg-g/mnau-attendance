...
type AdminUser=Option&{roles:string[];teacherId:string|null;studentId:string|null;curatorGroupIds:string[];deanFacultyIds:string[]};
const roles=[
...
export function AdminForm({actorId,users,groups,faculties,teachers,students}:{actorId:string;users:AdminUser[];groups:Option[];faculties:Option[];teachers:ProfileOption[];students:ProfileOption[]}){
  ...
  if(user){
    ...
    else if(action==='REMOVE_ROLE')targets=roles.filter(r=>user.roles.includes(r.id)&&!(r.id==='ADMIN'&&userId===actorId));
    ...
    else if(action==='ASSIGN_STAROSTA')targets=students.filter(s=>(!s.userId||s.userId===userId)&&(!user.studentId||s.id===user.studentId));
    else if(action==='REMOVE_STAROSTA')targets=students.filter(s=>s.id===user.studentId);
    else targets=[{id:'sessions',name:'Усі сесії користувача'}];
  }
  return <form className="admin-form pad" onSubmit={async e=>{
    ...
    try{
      ...
      setError(false);setMessage(data.changed?'Призначення оновлено. Зміну записано в аудит.':'Призначення вже має обраний стан.');router.refresh();
    ...
    <label>Дія<CustomSelect name="action" label="Дія" options={actions} value={action} onChange={setAction} disabled={busy} required/></label>
