...
export function ReportPreview({summary,filters,maxRows,path}:{summary:ReportSummary;filters:ReportFilters;maxRows?:number;path?:string}){
  ...
  const headers:[string,SortKey][]=[['Студент / студентка','report_student'],['Група','report_group'],['•','present'],['N','n'],['HV','hv'],['Не відмічено','unmarked'],['Чернетки','pending'],['Показник','report_percentage']];
  return <Panel title="Попередній перегляд звіту">
    ...
    {summary.stats&&<p className="panel-description">• {summary.stats.PRESENT} · N {summary.stats.N} · HV {summary.stats.HV} · Не відмічено: {summary.stats.unmarked} · Чернетки: {summary.stats.pending} · Заповнення: {percent(summary.stats.completion)}</p>}
