...
Спільний початковий beta-пароль: `Test1234!`. У БД зберігається bcrypt hash.

Існуючі `admin@test.com` та `dean@test.com` збережені. Цільовий `beta:repair-cs` відновлює погоджений пароль `Test1234!`, активність і beta-призначення для перелічених облікових записів. Повторний звичайний імпорт `beta:prepare-cs` зберігає змінені паролі та disabled accounts; це інша операція. `teacher@test.com` у перевіреній робочій БД відсутній.
