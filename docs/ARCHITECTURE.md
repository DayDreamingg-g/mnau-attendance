...
Единая метрика `PRESENT / (PRESENT + N) × 100` применяется после суммирования raw counts. HV исключён, нулевой знаменатель даёт null, пороги проверяются до округления. В аналитику входят подтверждённые отметки завершённых неотменённых пар; draft/unmarked и полнота показаны отдельно. Учёт времени использует Europe/Kyiv. Фиксированное 21:00 включается только при APP_ENV=demo + DEMO_MODE=true + валидной DEMO_DATE; пустая дата оставляет реальные часы. UI показывает только компактный DEMO; production/demo-конфликт отклоняется.
...
Дополнения: DEVELOPER/STUDENT, независимый StarostaAssignment, Student active/archivedAt/joinedAt/source/importKey, исторический LessonStudent.groupId и Attendance.isDemo. Полный контракт подготовки и scopes: [README](../README.md). Миграция сохраняет прежние roster group snapshots; последующие переводы их не изменяют. Normal seed отделён от явной destructive beta preparation.
