...
test('mixed curator/starosta permissions choose per-row confirmation',()=>{
  ...
  assert.equal(submissionMode([rows[0],rows[2]]),'CONFIRM');
  assert.equal(submissionMode([rows[1],rows[2]]),'DRAFT');
});
