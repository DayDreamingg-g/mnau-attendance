...
test('starosta saved marks immediately enter shared analytics and remain editable',async()=>{
  ...
  const after=await journal(teacher,liveId);assert.equal(after.rows[0].status,'N');assert.equal(after.rows[0].confirmed,true);const afterStats=await analytics(curator,f());assert.equal(afterStats.stats.N,beforeStats.stats.N+1);assert.equal(afterStats.stats.pending,beforeStats.stats.pending);
  await saveJournal(teacher,liveId,input(after.lesson.version,after.rows[0].id,'N'));
  const confirmed=await journal(starosta,liveId);assert.equal(confirmed.rows[0].confirmed,true);assert.equal(confirmed.rows[0].editable,true);await saveJournal(starosta,liveId,input(confirmed.lesson.version,confirmed.rows[0].id,'N','DRAFT'));
...
test('machine endpoint requires its own secret; reports contain real PDF/CSV and are deduplicated',async()=>{
  ...
  const [pdf,csv]=await Promise.all(['pdf','csv'].map(format=>fetch(`${base}/api/reports/${report.id}/${format}`,{headers:{cookie:auth.cookie}})));assert.equal(pdf.status,200);const pdfBytes=Buffer.from(await pdf.arrayBuffer());assert.equal(pdfBytes.subarray(0,5).toString(),'%PDF-');const csvText=await csv.text();assert.ok(csvText.includes('Дисципліна'));assert.ok(csvText.includes('Студент × заняття'));assert.ok(csvText.includes('Підсумок'));await mkdir('test-results',{recursive:true});await writeFile('test-results/sample-report.pdf',pdfBytes);
